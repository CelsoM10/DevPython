...
def deletar_insumo():
    if len(nome_insumo.get()) < 2:
        #tratar erro no input de dados:
        #Deletar a caixa de texto
        #Exibir msg do nome INvalido
        caixa_texto.delete("1.0", END)
        caixa_texto.insert("1.0", f" Nome Invalido!")
        return
    ...
    #para exibir o insumo deletado fizemos:
    caixa_texto.delete("1.0", END)
    #escrever na caixa de texto
...
def consumir_insumo():
    if len(nome_insumo.get()) < 1 or len(qtde_insumo.get()) < 1 or len(lote_insumo.get()) < 1:
        caixa_texto.delete("1.0", END)
    ...
    caixa_texto.delete("1.0", END)
    caixa_texto.insert("1.0",  f" Produto {nome_insumo.get()} teve {qtde_insumo.get()} Unidades consumidas!")
...
def visualizar_insumo():
    ...
    valores = cursor.fetchall() #--> retorna uma lista com tuplas, podemos fazer o unpanking das tuplas num loop
    texto = "" #--> definimos a variavel texto que será exibida
    for id_produtos, nome, quantidade, validade, lote in valores:
        texto = texto +f'''
        -----
        Produto    {nome}   
